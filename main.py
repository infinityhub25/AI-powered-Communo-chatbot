...
async def handle_request(request: Request):
    ...
    return JSONResponse(content={
        "fulfillmentText": "I didn't understand that 🤔"
    })
...
def add_to_order(parameters: dict, session_id: str):
    ...
    if len(food_items) != len(quantities):
        fulfillment_text = (
            ...
            "Can you please specify food items and quantities clearly?"
        )
    else:
...
def remove_from_order(parameters: dict, session_id: str):
    ...
    if session_id not in inprogress_orders:
        return JSONResponse(content={
            "fulfillmentText": (
                "I'm having a trouble finding your order. "
                "Sorry! Can you place a new order?"
            )
        })
    ...
    if len(current_order.keys()) == 0:
        ...
        fulfillment_text += (
            f" Here is what is left in your order: {order_str}"
        )
